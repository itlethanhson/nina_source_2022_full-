	...
	class Functions
	{
		...
		public function formenu($table,$type,$showh2=1)
		{
			...
			for($i=0;$i<count($danhmuc_cap1);$i++){
					$str.='<li><span></span><a class="has-child transition" href="'.$danhmuc_cap1[$i][$sluglang].'">'.$start_h2.$danhmuc_cap1[$i]["ten"].$end_h2.'</a>';
				...
				foreach($truyvan as $k => $v){
					...
					$str .='<div class="product">
			            <a class="box-product text-decoration-none" href="'.$v[$sluglang].'" title="'.$v['name'.$lang].'">
			                <p class="pic-product scale-img"><img onerror="this.src=\''.$dongdau.'assets/images/noimage.png\'" src="'.$dongdau.UPLOAD_PRODUCT_L.$v['photo'].'" alt="'.$v['name'.$lang].'"/></p>
			                <div class="info-product">
			                <h3 class="name-product text-split">'.$v['name'.$lang].'</h3>';
			                if(isset($config['sanpham']['mota']) && $config['sanpham']['mota'] == true){
				            $str .='<div class="desc-product text-split">'.$v['desc'.$lang].'</div>';
				        	}
			                $str .='<p class="price-product">';
			            ...
			            /*if(strpos($chuoi,'moi')==true) {
	                      $str .='<span class="price_per">New</span>';
	                    }*/
						...
						if(isset($config['sanpham']['muahang']) && $config['sanpham']['muahang'] == true){
		...
		function lay_baiviet($truyvan,$cssitem,$thumb){
				...
				foreach($truyvan as $k => $v){ ?>
					<a class="<?=$cssitem?> text-decoration-none w-clear" href="<?=$v[$sluglang]?>" title="<?=$v['name'.$lang]?>">
			            <p class="pic-news scale-img"><img onerror="this.src='<?=$thumb?>assets/images/noimage.png';" src="<?=$thumb?><?=UPLOAD_NEWS_L.$v['photo']?>" alt="<?=$v['name'.$lang]?>"></p>
			            <div class="info-news">
				...
				foreach($truyvan as $k => $v){ 
					...
					<div class="album">
						<a class=" text-decoration-none" data-fancybox="gallery<?=$v['id']?>" href="<?=UPLOAD_PRODUCT_L.$v['photo']?>" data-caption="<?=$v['name'.$lang]?>">
				            <p class="pic-album hover_sang2"><img onerror="this.src='<?=$thumb?>assets/images/noimage.png';" src="<?=$thumb?><?=UPLOAD_PRODUCT_L.$v['photo']?>" alt="<?=$v['name'.$lang]?>"/></p>
			       ...
			       <?php foreach($hinhanhsp as $k2 => $v2){ ?>
						...
						<a class=" text-decoration-none none" data-fancybox="gallery<?=$v['id']?>" href="<?=UPLOAD_PRODUCT_L.$v2['photo']?>" data-caption="<?=$v['name'.$lang]?>">
				            <p class="pic-album hover_sang2"><img onerror="this.src='<?=$thumb?>assets/images/noimage.png';" src="<?=$thumb?><?=UPLOAD_PRODUCT_L.$v2['photo']?>" alt="<?=$row_detail['name'.$lang]?>"/></p>
				        </a>
				    <?php } ?>
					</div>
				<?php  } }else{
				...
				foreach($truyvan as $k => $v){ 
					...
					<div class="album">
						<a class=" text-decoration-none" data-fancybox="gallery<?=$v['id']?>" href="<?=UPLOAD_PHOTO_L.$v['photo']?>" data-caption="<?=$v['name'.$lang]?>">
				            <p class="pic-album scale-img"><img onerror="this.src='<?=$thumb?>assets/images/noimage.png';" src="<?=$thumb?><?=UPLOAD_PHOTO_L.$v['photo']?>" alt="<?=$v['name'.$lang]?>"/></p>
				            <h3 class="name-album text-split"><span><?=$v['name'.$lang]?></span></h3>
				        </a>
					</div>
				<?php  }				
