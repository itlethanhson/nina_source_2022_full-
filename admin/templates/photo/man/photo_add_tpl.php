					    ...
					    <?php if(isset($config['photo']['man_photo'][$type]['check_photo'])) { foreach($config['photo']['man_photo'][$type]['check_photo'] as $key => $value) { ?>
					        <div class="form-group d-inline-block mb-2 mr-2">
					            ...
					            <div class="custom-control custom-checkbox d-inline-block align-middle">
					                <input type="checkbox" class="custom-control-input <?=$key?>-checkbox<?=$i?>" name="dataMulti[<?=$i?>][status][]" id="<?=$key?>-checkbox<?=$i?>" value="<?=$key?>" <?=($key=='hienthi') ? 'checked' : ''?>>
