<?php
	...
	function savePhotoStatic()
	{
		...
		else
		{
			...
			if((!empty($config['photo']['photo_static'][$type]['videomp4']) && $func->hasFile("file")) || (!empty($config['photo']['photo_static'][$type]['images']) && $func->hasFile("file")) || (isset($data['link_video']) && $data['link_video'] != ''))
			{
		...
		else
		{
			...
			if($numberPhoto && $dataMultiTemp)
			{
				...
				for($i=0;$i<count($dataMultiTemp);$i++)
				{
					...
					if(isset($config['photo']['man_photo'][$type]['nophoto']) && $config['photo']['man_photo'][$type]['nophoto'] == true)
					{
