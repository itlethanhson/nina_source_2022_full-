<?php
	...
	function saveMan()
	{
		...
		$row_opt = $d->rawQueryOne("select id, options,options2 from #_product where id = ? and type = ? limit 0,1",array($id_opt, $type));
		...
		if($id && $act!='save_copy')
		{
			...
			if($d->update('product',$data))
			{
				...
				if (isset($_POST['tentt'])) {
		        	for($m=0;$m<count($_POST['tentt']);$m++){
							$data31['type'] = $_POST['thuoctinh'][$m];							
							$data31['namevi'] = htmlspecialchars($func->sanitize($_POST['tentt'][$m]));
							$data31['thuoctinh1'] = htmlspecialchars($func->sanitize($_POST['thuoctinh1'][$m]));
							$data31['thuoctinh2'] = htmlspecialchars($func->sanitize($_POST['thuoctinh2'][$m]));
							$data31['regular_price'] = str_replace(",","",$_POST['giatt'][$m]) ;
							$data31['sale_price'] = str_replace(",","",$_POST['giakmtt'][$m]) ;
							$data31['id_product'] = $id;
							$d->insert('product_size_mau',$data31);							
		        	}
		        }
		...
		{	
			...
			/*lay stt*/
			$sott = $d->rawQuery("select numb from #_product where type = ? order by numb desc ",array($type));
			$sottmoi = $sott[0]['numb']+1;
			...
			if($d->insert('product',$data))
			{
				...
				if (isset($_POST['tentt'])) {
		        	for($m=0;$m<count($_POST['tentt']);$m++){
							...
							$data31['regular_price'] = str_replace(",","",$_POST['giatt'][$m]) ;
							$data31['sale_price'] = str_replace(",","",$_POST['giakmtt'][$m]) ;
							$data31['id_product'] = $id_insert;
							$d->insert('product_size_mau',$data31);							
		        	}
		        }

				/*update stt*/
				$d->rawQuery("update #_product set numb = ? where id = ".$id_insert,array($sottmoi));
