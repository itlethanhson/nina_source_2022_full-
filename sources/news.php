<?php  
	...
	else
	{
		...
		if($_GET['idthe']!=''||$_GET['ten']!=''){

			$where_tbh = "";
			if($_GET['idthe']!='')
			$where_tbh .= "mathe = '".$_GET['idthe']."' and ";
			if($_GET['ten']!='')
			$where_tbh .= "tenvi = '".$_GET['ten']."' and ";
			$where_tbh .= " type = ? and hienthi > 0";
			$params_tbh = array('the-bao-hanh');

			$sqlthe = "select ten$lang, options2, mathe, type from #_news where $where_tbh order by stt,id desc";
			$thebaohanh = $d->rawQuery($sqlthe,$params_tbh);
